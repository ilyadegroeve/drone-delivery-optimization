...
def get_all_partitions(items, max_parts=None):
    ...
    for smaller in get_all_partitions(items[1:], max_parts):
        # Add first to each part of smaller
        if max_parts is None or len(smaller) < max_parts:
            yield [[first]] + smaller
        # Add first to each smaller part
...
def create_subroutes(partition):
    ...
    for part in partition:
        if part:  # Only create subroutes for non-empty parts
            subroute = ["VUB"] + part + ["VUB"]
...
def optimize_route():
    ...
    for perm in itertools.permutations(hospitals):
        ...
        for max_parts in range(1, len(hospitals) + 1):
            for partition in get_all_partitions(list(perm), max_parts):
                subroutes = create_subroutes(partition)
                consumption = total_current_with_subroutes(subroutes)
